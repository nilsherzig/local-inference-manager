...
// hfCLI is the HuggingFace downloader binary. The image installs it via
// huggingface_hub[cli]; the classic name still works on newer releases.
const hfCLI = "huggingface-cli"

// hfTransferEnv enables the Rust hf_transfer backend, which splits each file
// into chunks and downloads them in parallel. On a high-bandwidth link this
// lifts the single-stream cap (~10MB/s in practice) to >1GB/s. The plain CLI is
// otherwise capped because it fetches a file with a single connection.
const hfTransferEnv = "HF_HUB_ENABLE_HF_TRANSFER=1"
...
// progress bar, fast hf_transfer backend) instead of blocking the first user
// request. huggingface-cli is idempotent: files already on disk are verified by
// etag and skipped, so restarts stay fast. Models with no `downloads` entries
// are assumed to already have their -m paths on disk and are skipped.
//
...
// hfDownload runs `huggingface-cli download` for one entry, streaming its output
// (including the progress bar) straight to the manager's stdout/stderr so the
// user sees live download progress during startup. It is the default value of
...
func (m *Manager) hfDownload(d config.Download) error {
	...
	cmd.Env = append(os.Environ(), hfTransferEnv)
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	return cmd.Run()
}

// downloadArgs builds the huggingface-cli argv for one entry. A quant tag is
